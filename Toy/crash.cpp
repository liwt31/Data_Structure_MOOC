...
class Queue{
    ...
    Qnode<T>* end_ptr;
public:
    Queue<T>():front_ptr(NULL),end_ptr(NULL){};
...
template<typename T>
T Queue<T>::Pop(){
    fout<<"Pop() entered"<<endl;
    if(front_ptr==end_ptr){
        end_ptr=NULL;
    }
    fout<<"single element judge done"<<endl;
    Qnode<T> *old_front=front_ptr;
    front_ptr=front_ptr->next;
    T bak=old_front->data;
    fout<<"pointer setting done"<<endl;
    Qnode<T> *ptr_bak=old_front;
    fout<<"bak of deleting "<<ptr_bak<<endl;
    fout<<"deleting "<<old_front<<" ";
    fout<<"deleting "<<old_front<<" ";
    delete ptr_bak;
    fout<<"delete done"<<endl;
    old_front=NULL;
    return bak;
}
...
void Toy::TransformB(Toy &new_toy)const{
    int temp0=new_toy.toy[0][3];
    int temp1=new_toy.toy[1][3];
    for(int i=2;i>=0;i--){
        new_toy.toy[0][i+1]=new_toy.toy[0][i];
        new_toy.toy[1][i+1]=new_toy.toy[1][i];
    }
    new_toy.toy[0][0]=temp0;
    new_toy.toy[1][0]=temp1;
}

void Toy::TransformC(Toy &new_toy)const{
    int temp=new_toy.toy[0][1];
    new_toy.toy[0][1]=new_toy.toy[1][1];
    new_toy.toy[1][1]=new_toy.toy[1][2];
    new_toy.toy[1][2]=new_toy.toy[0][2];
    new_toy.toy[0][2]=temp;
}
...
//        int temp;scanf("%d",&temp);playing.SetToy(i,temp);
    ...
    q_toy.Push(playing);
    while(!q_toy.IsEmpty()){
        fout<<"preparing enter Pop()"<<endl;
        Toy parent=q_toy.Pop();
        fout<<"out of Pop()"<<endl;
        for(int i=0;i<3;i++){
            fout<<"enter trans cycle"<<endl;
...
//                child.print();
    ...
    for(int i=0;i<N;i++){
        ...
        for(int j=0;j<8;j++){
            ...
            fout<<"SetToy done"<<endl;
        }
        fout<<"out of set cycle"<<endl;
